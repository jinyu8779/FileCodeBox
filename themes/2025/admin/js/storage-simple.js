...
function fillStorageConfigForms(storageConfig) {
    ...
    const dirStructure = document.getElementById('local-dir-structure');
    if (dirStructure) {
        const style = (storageConfig.dir_structure || 'none').toLowerCase();
        dirStructure.value = style === 'date' ? 'date' : 'none';
    }
...
function getStorageConfigByType(type) {
    ...
    switch (type) {
        case 'local':
            ...
            const dirStructure = document.getElementById('local-dir-structure');
            if (dirStructure) {
                config.dir_structure = dirStructure.value || 'none';
            }
            break;
