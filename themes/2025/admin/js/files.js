...
function displayFilesList(files) {
    ...
    tbody.innerHTML = files.map(file => {
        ...
        return '<tr>' +
            '<td><input type="checkbox" class="file-checkbox" value="' + file.code + '" onchange="toggleFileSelection(\'' + file.code + '\')"></td>' +
...
function displayFilesGrid(files) {
    ...
    gridContainer.innerHTML = files.map(file => {
        ...
        return '<div class="file-card" onclick="selectFileCard(\'' + file.code + '\')">' +
...
async function downloadFile(fileCode) {
    try {
        const response = await fetch('/admin/files/download?code=' + encodeURIComponent(fileCode), {
            headers: {
                'Authorization': 'Bearer ' + (window.authToken || localStorage.getItem('user_token') || '')
            }
        });
...
async function copyFileLink(fileCode) {
    try {
        const link = window.location.origin + '/share/download?code=' + encodeURIComponent(fileCode);
...
async function deleteFile(fileCode) {
    if (!fileCode) {
        FilesManager.showAlert('缺少文件提取码', 'error');
        return;
    }
    ...
    try {
        ...
        if (result.code === 200) {
            ...
            FilesManager.selectedFiles.delete(fileCode);
            updateBulkActions();
            loadFiles(FilesManager.currentPage, FilesManager.currentSearch, FilesManager.currentFilter);
        } else {
...
async function bulkDelete() {
    ...
    for (const code of selectedFiles) {
        try {
            const result = await apiRequest('/admin/files/' + encodeURIComponent(code), {
                method: 'DELETE'
            });
            if (result.code === 200) {
                successCount++;
            ...
            console.error('删除文件失败:', code, error);
            failCount++;
        }
    }
    ...
    FilesManager.selectedFiles.clear();
    updateBulkActions();
    await loadFiles(FilesManager.currentPage, FilesManager.currentSearch, FilesManager.currentFilter);
    
    if (failCount === 0) {
        FilesManager.showAlert(`已成功删除 ${successCount} 个文件`, 'success');
    } else {
        FilesManager.showAlert(`删除完成：成功 ${successCount} 个，失败 ${failCount} 个`, 'warning');
    }
}
