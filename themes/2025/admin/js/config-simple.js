...
function fillConfigForm(config) {
    ...
    // 管理员令牌字段已移除，不回显任何敏感字段
        ...
        setFieldValue('max_folder_files', config.transfer?.upload?.max_folder_files ?? 100);
        setFieldValue('share_response_type', config.transfer?.upload?.share_response_type || 'code');
        setFieldValue('default_expire_style', config.transfer?.upload?.default_expire_style || 'day');
        setFieldValue('default_expire_value', config.transfer?.upload?.default_expire_value ?? 1);
        // nil/undefined 视为允许修改
        const allowModify = config.transfer?.upload?.allow_modify_expire;
        setCheckboxValue('allow_modify_expire', allowModify === undefined || allowModify === null ? 1 : allowModify);
        ...
        const config = {
            ...
            transfer: {
                upload: {
                    ...
                    max_folder_files: getFieldValue('max_folder_files', 'number'),
                    share_response_type: getFieldValue('share_response_type') || 'code',
                    default_expire_style: getFieldValue('default_expire_style') || 'day',
                    default_expire_value: getFieldValue('default_expire_value', 'number') ?? 1,
                    allow_modify_expire: getCheckboxValue('allow_modify_expire') ? 1 : 0
                },
                download: {
