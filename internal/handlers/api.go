...
type SystemConfig struct {
	...
	MaxFolderFiles     int      `json:"maxFolderFiles" example:"100"`
	ShareResponseType  string   `json:"shareResponseType" example:"code"`
	RequireLogin       int      `json:"requireLogin" example:"0"`
	DefaultExpireValue int      `json:"defaultExpireValue" example:"1"`
	DefaultExpireStyle string   `json:"defaultExpireStyle" example:"day"`
	AllowModifyExpire  int      `json:"allowModifyExpire" example:"1"`
}
...
func (h *APIHandler) GetConfig(c *gin.Context) {
	responseType := h.config.Transfer.Upload.ShareResponseType
	if responseType != "url" {
		responseType = "code"
	}
	systemConfig := SystemConfig{
		...
		MaxFolderFiles:     h.config.Transfer.Upload.MaxFolderFiles,
		ShareResponseType:  responseType,
		RequireLogin:       h.config.Transfer.Upload.RequireLogin,
		DefaultExpireValue: h.config.Transfer.Upload.GetDefaultExpireValue(),
		DefaultExpireStyle: h.config.Transfer.Upload.GetDefaultExpireStyle(),
		AllowModifyExpire:  h.config.Transfer.Upload.GetAllowModifyExpire(),
	}
