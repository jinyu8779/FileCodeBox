...
func (s *Service) DeleteFile(id uint) error {
	...
	return s.deleteFileRecord(fileCode)
}
...
// deleteFileRecord 先删物理文件，再删数据库记录。
// 物理文件找不到时仍清理库记录（孤儿数据）；删除操作本身失败则中止，避免只删库。
func (s *Service) deleteFileRecord(fileCode *models.FileCode) error {
	result := s.storageService.DeleteFileWithResult(fileCode)
	if !result.Success {
		if result.Error != nil && errors.Is(result.Error, storage.ErrPhysicalFileNotFound) {
			logrus.WithError(result.Error).
				WithField("code", fileCode.Code).
				WithField("file_path", fileCode.FilePath).
				WithField("uuid_file_name", fileCode.UUIDFileName).
				Warn("physical file missing, continue removing database record")
		} else {
			errMsg := "删除物理文件失败"
			if result.Error != nil {
				errMsg = fmt.Sprintf("%s: %v", errMsg, result.Error)
