	...
	return s.manager.UpdateTransaction(func(draft *config.ConfigManager) error {
		...
		if configRequest.Transfer != nil {
			if configRequest.Transfer.Upload != nil {
				...
				draft.Transfer.Upload.MaxFolderFiles = uploadConfig.MaxFolderFiles
				if uploadConfig.ShareResponseType != "" {
					draft.Transfer.Upload.ShareResponseType = uploadConfig.ShareResponseType
				}
				if uploadConfig.DefaultExpireStyle != "" {
					draft.Transfer.Upload.DefaultExpireStyle = uploadConfig.DefaultExpireStyle
				}
				if uploadConfig.DefaultExpireValue > 0 || uploadConfig.DefaultExpireStyle == "forever" {
					draft.Transfer.Upload.DefaultExpireValue = uploadConfig.DefaultExpireValue
				}
				if uploadConfig.AllowModifyExpire != nil {
					v := *uploadConfig.AllowModifyExpire
					draft.Transfer.Upload.AllowModifyExpire = &v
				}
			}
		...
		if configRequest.Storage != nil {
			...
			draft.Storage.DirStructure = storageConfig.DirStructure
