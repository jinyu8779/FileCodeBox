...
func (s *Service) DeleteFileShare(code string) error {
	...
	if !result.Success {
		if result.Error == nil || !errors.Is(result.Error, storage.ErrPhysicalFileNotFound) {
			if result.Error != nil {
				return fmt.Errorf("删除物理文件失败: %w", result.Error)
			}
			return fmt.Errorf("删除物理文件失败: %s", result.Message)
		}
		logrus.WithError(result.Error).
			WithField("code", fileCode.Code).
			Warn("physical file missing during share removal, continue removing database record")
	}
