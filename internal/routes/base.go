...
func SetupBaseRoutes(router *gin.Engine, userHandler *handlers.UserHandler, cfg *config.ConfigManager) {
	...
	router.POST("/", func(c *gin.Context) {
		responseType := cfg.Transfer.Upload.ShareResponseType
		if responseType != "url" {
			responseType = "code"
		}
		common.SuccessResponse(c, gin.H{
			...
			"maxFolderFiles":     cfg.Transfer.Upload.MaxFolderFiles,
			"shareResponseType":  responseType,
			"requireLogin":       cfg.Transfer.Upload.RequireLogin,
			"defaultExpireValue": cfg.Transfer.Upload.GetDefaultExpireValue(),
			"defaultExpireStyle": cfg.Transfer.Upload.GetDefaultExpireStyle(),
			"allowModifyExpire":  cfg.Transfer.Upload.GetAllowModifyExpire(),
		})
	})
}
