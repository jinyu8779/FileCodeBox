...
func (sc *StorageConfig) Validate() error {
	...
	style := strings.ToLower(strings.TrimSpace(sc.DirStructure))
	if style == "" {
		sc.DirStructure = "none"
	} else if style != "none" && style != "date" {
		return fmt.Errorf("文件存储目录结构只能是 none 或 date")
	} else {
		sc.DirStructure = style
	}
...
// GetDirStructure 获取规范化后的目录结构配置（none/date）
func (sc *StorageConfig) GetDirStructure() string {
	if sc == nil {
		return "none"
	}
	style := strings.ToLower(strings.TrimSpace(sc.DirStructure))
